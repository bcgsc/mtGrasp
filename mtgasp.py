...
parser.add_argument('-t', '--threads', help='Number of threads', nargs='?', const=1, default = 3)
parser.add_argument('-k', '--kmer', help='k-mer size used in abyss de novo assembly', nargs='?', const=1, default = 96)
parser.add_argument('-c', '--kc', help='kc', nargs='?', const=1, default = 3)
parser.add_argument('-r', '--ref_path', help='k-mer size', required=True)
parser.add_argument('-n', '--dry_run', help='Dry-run pipeline', action='store_true')
parser.add_argument('-a', '--abyss_fpr', help='False positive rate for the bloom filter used by abyss', nargs='?', const=1, default = 0.005)
parser.add_argument('-s', '--sealer_fpr', help='False positive rate for the bloom filter used by sealer during gap filling', nargs='?', const=1, default = 0.01)
parser.add_argument('-p', '--gap_filling_p', help='Merge at most N alternate paths during sealer gap filling step', nargs='?', const=1, default = 5)
parser.add_argument('-b', '--sealer_k', help='k-mer size used in sealer gap filling', nargs='?', const=1, default = '60,80,100,120')
parser.add_argument('-e', '--end_recov_sealer_fpr', help='False positive rate for the bloom filter used by sealer during flanking end recovery', nargs='?', const=1, default = 0.01)
parser.add_argument('-v', '--end_recov_sealer_k', help='k-mer size used in sealer flanking end recovery', nargs='?', const=1, default = '60,80,100,120')
parser.add_argument('-i', '--end_recov_p', help='Merge at most N alternate paths during sealer flanking end recovery', nargs='?', const=1, default = 5)
