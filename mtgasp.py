...
parser.add_argument('-t', '--threads', help='Number of threads', default = 3)
parser.add_argument('-k', '--kmer', help='k-mer size used in abyss de novo assembly', default = 96)
parser.add_argument('-c', '--kc', help='kc', default = 3)
...
parser.add_argument('-a', '--abyss_fpr', help='False positive rate for the bloom filter used by abyss', default = 0.005)
parser.add_argument('-s', '--sealer_fpr', help='False positive rate for the bloom filter used by sealer during gap filling', default = 0.01)
parser.add_argument('-p', '--gap_filling_p', help='Merge at most N alternate paths during sealer gap filling step', default = 5)
parser.add_argument('-b', '--sealer_k', help='k-mer size used in sealer gap filling', default = '60,80,100,120')
parser.add_argument('-e', '--end_recov_sealer_fpr', help='False positive rate for the bloom filter used by sealer during flanking end recovery', default = 0.01)
parser.add_argument('-v', '--end_recov_sealer_k', help='k-mer size used in sealer flanking end recovery', default = '60,80,100,120')
parser.add_argument('-i', '--end_recov_p', help='Merge at most N alternate paths during sealer flanking end recovery', default = 5)
